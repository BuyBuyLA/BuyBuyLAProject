...
<%--  <sql:query sql="select * from Cart" var="rs" dataSource="${snapshot}" />  --%>
	...
	<div class="container">
		...
		<table>
			...
			<tbody id="tbody">
			<c:forEach var="additems" items="${additem}">
				<tr>
					<td><img width='100' height='200' src="<c:url value='/getPicture/${additems.PID}' />" /></td>
					<td>${additems.P_name}</td>
					<td><a href="<c:url value='/sub' />?id=${additems.PID}"><button class="down">-</button></a><input type="text" value="${row.count}" readonly="readonly" /><a href="<c:url value='/add' />?id=${row.PID}"><button class="up">+</button></a></td>
					<td>NT<span>${additems.P_price}</span></td>
					<td>NT<span class="total">${additems.count*row.P_price}</span></td>
					<td>
